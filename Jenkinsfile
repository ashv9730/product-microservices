    ...
    stages{
        ...
        stage("cloning Git Repo"){
            steps{
                ...
                git url: "https://github.com/ashv9730/product-microservices.git", branch: "master"
            }
            post{
                always{
        ...
        stage("Build stage"){
            steps{
                ...
                sh 'docker build -t product-microservices .'
            }
            post{
                always{
        ...
        stage("Push Image in Docker HUb"){
            steps{
                ...
                withCredentials([usernamePassword(credentialsId: 'dockerhub-cred', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                    sh "docker tag product-microservices ${env.USERNAME}/product-microservices:${env.VERSION}"
                    sh "docker login -u ${env.USERNAME} -p ${env.PASSWORD}" 
                    sh "docker push ${env.USERNAME}/product-microservices:${env.VERSION}"
                }
            }
            post{
                always{
